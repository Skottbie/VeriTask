...
def _load_default_escrow() -> str:
    ...
    escrow = (data.get("xlayer") or {}).get("VeriEscrow")
    if not isinstance(escrow, str) or not Web3.is_address(escrow):
        raise ValueError("Escrow missing or invalid in addresses.json[xlayer].VeriEscrow")
    return escrow
...
def main() -> None:
    ...
    parser.add_argument("--escrow", default="", help="VeriEscrow address (optional, defaults to addresses.json xlayer)")
