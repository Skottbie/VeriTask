...
name: launch-agent
description: >
  发射 AI Agent。创建 OKX Agentic Wallet + 注册到 VTRegistry 链上合约。
...
> **角色**: AI Agent 发射器 — 为新 Agent 创建 OKX Agentic Wallet 并注册到 X Layer 链上
...
```
python3 {baseDir}/launch_agent.py --name "<AGENT_NAME>" --description "<AGENT_DESCRIPTION>" --claims "<CLAIM1>,<CLAIM2>" --registry 0x02eDBBB5ECDb56aCB4b25CFAF7279f8E6D81F8E4 --json
```
...
- `--registry`: VTRegistry 合约地址（当前主网: `0x02eDBBB5ECDb56aCB4b25CFAF7279f8E6D81F8E4`）
- `--json`: 机器可读 JSON 输出
...
用户: "发射一个 Agent"
  ...
  │   内部流程:
  │   1. 解析钱包模式
  │      - dedicated: onchainos wallet add（创建 Agent 专属钱包）
  │      - fixed: 直接使用固定钱包地址
