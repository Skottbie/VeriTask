...
```
python3 {baseDir}/invest_agent.py --agent-id <AGENT_ID> --amount <USDT_AMOUNT> --escrow 0x9c2B78E6F3499b8a7aC356c8fFaCEe95939aC1b2 --json
```

**参数**:
- `--agent-id`: Agent ID（从 VTRegistry 获取，如 `3`）— **必需**
- `--amount`: 投资金额（USDT，如 `10`）— **必需**
- `--escrow`: VeriEscrow 合约地址 — **当前主网**: `0x9c2B78E6F3499b8a7aC356c8fFaCEe95939aC1b2`
- `--usdt`: USDT 合约地址（默认 X Layer: `0x779ded0c9e1022225f8e0630b35a9b54be713736`，无需指定）
- `--json`: 机器可读 JSON 输出（**始终加上**）

**⚠️ 重要**: 收到用户投资请求后，**立即** exec 上述命令。不要手动调用 onchainos。
...
> **参考 VeriTask task-delegator Step 0c 模式**

**0c-1. 扫描可用资产：**
```bash
onchainos portfolio all-balances --address <BACKER_ADDRESS> --chains xlayer
```

**0c-2. 获取换币报价：**
如果有非 USDT 资产且价值足够，获取报价：
```bash
onchainos swap quote --from <TOKEN_ADDRESS> --to 0x779ded0c9e1022225f8e0630b35a9b54be713736 --amount <AMOUNT> --chain xlayer
```
...
⛔ **禁止行为**：
- 禁止手动调用 onchainos wallet switch/status/contract-call 等钱包管理命令
