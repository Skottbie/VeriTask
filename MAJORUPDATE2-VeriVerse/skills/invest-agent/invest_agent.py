...
Flow:
  ...
  2. Check USDT balance via onchainos portfolio (read-only)
  3. Sign approve(escrow, amount) locally → broadcast via onchainos gateway
  4. Sign invest(agentId, amount) locally → broadcast via onchainos gateway

Pattern follows VeriTask swap_and_broadcast.py:
  CLIENT_PRIVATE_KEY sign → onchainos gateway broadcast
"""
...
def _run_onchainos(args: list[str]) -> dict:
    ...
    if result.returncode != 0:
        ...
        _err(f"onchainos failed (exit {result.returncode}): {err}")
...
def _get_nonce(address: str) -> int:
    """Get current nonce via X Layer JSON-RPC."""
    resp = requests.post(
        RPC_URL,
...
def _get_gas_price() -> int:
    """Get current gas price via X Layer JSON-RPC."""
    resp = requests.post(
        RPC_URL,
...
def _get_tx_receipt(tx_hash: str) -> dict[str, Any] | None:
    """Fetch transaction receipt from X Layer JSON-RPC."""
    resp = requests.post(
        RPC_URL,
...
def _eth_call(to_address: str, data_hex: str) -> str:
    """Run eth_call against X Layer JSON-RPC and return raw hex result."""
    resp = requests.post(
        RPC_URL,
        json={
            "jsonrpc": "2.0",
...
def _extract_tx_hash(result: dict) -> str | None:
    """Extract tx hash from onchainos broadcast response."""
    if not isinstance(result, dict):
        return None
    bd = result.get("data", [])
    if isinstance(bd, list) and bd:
        return bd[0].get("txHash") or bd[0].get("orderId")
    if isinstance(bd, dict):
        return bd.get("txHash") or bd.get("orderId")
    return None
...
def _extract_risk_action(scan_result: Any) -> str:
    """Extract security action from varied onchainos result shapes."""
...
def _simulate_tx(from_addr: str, to_addr: str, data_hex: str, amount_raw: int = 0) -> dict:
    """Run onchainos gateway simulate as a pre-broadcast guardrail."""
    ...
    if isinstance(result, dict) and result.get("ok") is False:
        return {"ok": False, "error": result.get("error", "simulate failed"), "raw": result}
...
def _security_scan_tx(from_addr: str, to_addr: str, data_hex: str, value_raw: int = 0) -> dict:
    """Run onchainos security tx-scan with fail-safe handling."""
    result = _run_onchainos([
        "security", "tx-scan",
        "--chain", CHAIN_NAME,
        "--from", from_addr,
    ...
    if isinstance(result, dict) and result.get("ok") is False:
        return {"ok": False, "error": result.get("error", "tx-scan failed"), "raw": result}

    action = _extract_risk_action(result)
    if action == "block":
        return {"ok": False, "error": "security action=block", "raw": result}
    if action == "warn" and not INVEST_ALLOW_WARN_RISK:
        return {"ok": False, "error": "security action=warn (blocked by policy)", "raw": result}
...
def check_usdt_balance(wallet_address: str) -> float | None:
    """Check USDT balance on X Layer via onchainos portfolio (read-only).
    Returns balance in UI units, or None if query failed."""
    try:
        result = _run_onchainos([
            "portfolio", "token-balances",
            "--address", wallet_address,
            "--tokens", f"{CHAIN_INDEX}:{USDT_ADDRESS}",
        ])
        if isinstance(result, list):
            for token in result:
                if token.get("tokenContractAddress", "").lower() == USDT_ADDRESS.lower():
                    return float(token.get("balance", "0"))
        elif isinstance(result, dict) and result.get("ok") and "data" in result:
            data = result["data"]
            if isinstance(data, list):
                for entry in data:
                    token_list = entry.get("tokenAssets", [])
                    for token in token_list:
                        if token.get("tokenContractAddress", "").lower() == USDT_ADDRESS.lower():
                            return float(token.get("balance", "0"))
            else:
                token_list = data.get("tokenAssets", [])
                for token in token_list:
                    if token.get("tokenContractAddress", "").lower() == USDT_ADDRESS.lower():
                        return float(token.get("balance", "0"))
        return 0.0
...
def invest_agent(agent_id: int, amount_usdt: float, escrow_address: str, usdt_address: str) -> dict:
    ...
    2. Check USDT balance via onchainos portfolio (read-only)
    3. Sign approve(escrow, amount) → broadcast via onchainos gateway
    4. Sign invest(agentId, amount) → broadcast via onchainos gateway
    """
...
def main():
    ...
    parser.add_argument("--usdt", default="0x779ded0c9e1022225f8e0630b35a9b54be713736",
                        help="USDT contract address on X Layer")
