...
if X402_PAYER_DIR.exists():
    sys.path.insert(0, str(X402_PAYER_DIR))

try:
    from okx_x402_payer import execute_payment  # type: ignore
except Exception as exc:
    raise RuntimeError(f"Failed to import x402 payer module from {X402_PAYER_DIR}: {exc}")
...
DEFAULT_RPC_URL = os.getenv("XLAYER_RPC_URL", "https://rpc.xlayer.tech")
...
XLAYER_CHAIN_INDEX = os.getenv("CHAIN_INDEX", "196")
ONCHAINOS_BIN = os.getenv("ONCHAINOS_BIN", os.path.expanduser("~/.local/bin/onchainos"))
CHAIN_NAME = os.getenv("CHAIN_NAME", "xlayer")
...
def _run_onchainos_json(args: list[str], timeout: int = 30) -> Any:
    ...
    try:
        ...
        preview = raw[:300]
        raise RuntimeError(f"onchainos returned invalid JSON: {preview}") from exc
...
def _security_scan_tx(from_addr: str, to_addr: str, data_hex: str, value_raw: int = 0) -> dict[str, Any]:
    ...
    action = _extract_risk_action(result)
    if action == "block":
        return {"ok": False, "error": "security action=block", "action": action, "raw": result}
    if action == "warn" and not CHALLENGE_ALLOW_WARN_RISK:
        return {
            "ok": False,
            "error": "security action=warn (blocked by policy)",
            "action": action,
            "raw": result,
        }

    return {"ok": True, "action": action, "raw": result}
...
def _simulate_tx(from_addr: str, to_addr: str, data_hex: str, amount_raw: int = 0) -> dict[str, Any]:
    ...
    fail_reason = _extract_fail_reason(result)
    if fail_reason:
        return {"ok": False, "error": f"simulate failReason: {fail_reason}", "raw": result}
...
def _broadcast_signed_tx(signed_tx_hex: str, wallet_address: str) -> dict[str, Any]:
    ...
    tx_hash = _extract_gateway_tx_hash(result)
    order_id = _extract_gateway_order_id(result)
    if not tx_hash and not order_id:
        return {"ok": False, "error": "gateway broadcast missing txHash/orderId", "raw": result}
...
def _query_gateway_order(wallet_address: str, order_id: str) -> dict[str, Any]:
    ...
    return {
        "txHash": _extract_gateway_tx_hash(result),
        "status": _extract_gateway_order_status(result),
        "raw": result,
    }
...
def _precheck_challenger_balance(required_usdt: float) -> dict[str, Any]:
    ...
    payload = _run_onchainos_json(
        [
            ...
            wallet,
            "--tokens",
            f"{XLAYER_CHAIN_INDEX}:{USDT_TOKEN_CONTRACT}",
        ]
    )
    balance_usdt = _extract_usdt_balance_from_portfolio(payload, USDT_TOKEN_CONTRACT)
    sufficient = balance_usdt >= required_usdt

    return {
        "wallet": wallet,
        "chainIndex": XLAYER_CHAIN_INDEX,
        "usdtContract": Web3.to_checksum_address(USDT_TOKEN_CONTRACT),
...
def _load_registry_address(explicit: str | None) -> str:
    ...
    if addresses_path.exists():
        ...
        xlayer = data.get("xlayer", {})
        addr = xlayer.get("VTRegistry")
        if addr:
...
def _resolve_verifier_payees(dao_payload: dict[str, Any]) -> list[str]:
    fallback_a = os.getenv("DAO_WALLET_A", "").strip()
    fallback_b = os.getenv("DAO_WALLET_B", "").strip()

    va = dao_payload.get("verifier_a", {})
    vb = dao_payload.get("verifier_b", {})

    candidate_a = str((va.get("payeeAddress") if isinstance(va, dict) else "") or fallback_a).strip()
    candidate_b = str((vb.get("payeeAddress") if isinstance(vb, dict) else "") or fallback_b).strip()

    if not candidate_a:
        raise ValueError("Verifier A payee address missing (dao.verifier_a.payeeAddress or DAO_WALLET_A)")
    if not candidate_b:
        raise ValueError("Verifier B payee address missing (dao.verifier_b.payeeAddress or DAO_WALLET_B)")
...
def _x402_pay_with_retry(payee: str, amount_usdt: float, max_retries: int, delay_ms: int) -> dict[str, Any]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            result = execute_payment(payee, amount_usdt)
            ...
            last_error = result.get("error", "x402 payment returned success=false")
        except Exception as exc:
            last_error = str(exc)
    ...
    return {
        ...
        "error": str(last_error or "x402 payment failed"),
    }
...
def finalize_trust_update(
    ...
    payment: dict[str, Any] = {
        "eligible": bool(trusted_valid and provenance_valid),
        "attempted": False,
        "success": False,
...
def main() -> None:
    ...
    parser.add_argument("--rpc-url", default=DEFAULT_RPC_URL, help="X Layer RPC URL")
