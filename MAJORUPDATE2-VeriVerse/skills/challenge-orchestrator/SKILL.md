...
python3 {baseDir}/challenge_orchestrator.py \
  --agent-id <AGENT_ID> \
  --registry 0x02eDBBB5ECDb56aCB4b25CFAF7279f8E6D81F8E4 \
...
说明：
- 该命令不会上链，只做 reviewer_provenance 合规性预检。
- 当 `reviewerProvenanceValid=false` 时，必须先重做评审输出，禁止直接执行 Command C。
...
## Step 0c: 余额不足处理（与 VeriTask / invest-agent 对齐）
...
```bash
onchainos portfolio all-balances --address <CHALLENGER_WALLET> --chains xlayer
```

2. 兑换报价（只选可兑换且价值足够的非 USDT 资产）：

```bash
onchainos swap quote --from <TOKEN_ADDRESS> --to 0x779ded0c9e1022225f8e0630b35a9b54be713736 --amount <MINIMAL_UNITS> --chain xlayer
```

2b. 如需评审链路证据，提取路由来源（M4）：

```bash
python3 {baseDir}/../uniswap-evidence/uniswap_evidence.py \
...
8. 将 Command B 的 `trustedLayer.sourceHash` 透传给 Command V（`--source-hash-expected`）先预检。
9. 仅当 Command V 返回 `reviewerProvenanceValid=true` 时，才允许执行 Command C 上链。
